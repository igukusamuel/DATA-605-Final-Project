...
# Probability of X is higher than x, the median of X given that X is greater than y, the first quantile of Y.
...
*Since the p-value is significantly high, we fail to reject the null hypothesis.*
...
*The tests above for pairwise corelation using pearson method estimated the association between the paired samples and computed a test of the value*
...
*respective correlation coefficient shown above.*
...
Conduct LU decomposition on the matrix. (5 points)
...
assume_normality = rnorm(length(z), mean(z), sd(z))
...
Finally, provide the empirical 5th percentile and 95th percentile of the data.  Discuss. (5 points)
...
Kaggle Username is *Samuel Kigamba*
